-- ═══════════════════════════════════════════════════════════════════════════
--  « LE SEMESTRE » — où en est chaque séance, d'un coup d'œil
--
--  Le seul angle mort qui restait : le portail savait tout dire d'UNE séance,
--  rien de l'ensemble. Savoir si la S7 a son corrigé se lisait dans un tableau
--  Drive tenu à la main, donc faux au bout de trois semaines.
--
--  Une ligne par séance, l'état calculé depuis la base :
--
--    à produire   aucun corrigé — la séance existe, mais rien ne l'évaluera
--    prête        corrigés en place, personne n'a encore répondu
--    en cours     ouverte, et des réponses arrivent
--    jouée        fermée, avec des réponses
--
--  Les séances 90 et au-delà sont exclues : l'appel, la connaissance et le
--  stage ne sont pas des séances du semestre, et les mêler brouillerait la
--  seule question à laquelle ce tableau répond — où en est la progression.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  parseance as (
    select s.id, s.classe_id, s.numero, s.titre, s.nature, s.notee, s.ouverte,
           s.jalons, s.echeance, s.duree_min,
...
-- ─── Contrôle ──────────────────────────────────────────────────────────────
select c->>'code' as classe, c->>'total' as seances, c->>'pretes' as pretes
  from jsonb_array_elements(semestre()->'classes') c;
